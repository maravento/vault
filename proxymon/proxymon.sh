...
check_dependencies() {
    ...
    pkgs='wget git tar ipset libnotify-bin nbtscan libcgi-session-perl libgd-perl python-is-python3 coreutils sarg php8.3 libapache2-mod-php8.3 php8.3-cli fonts-lato fonts-liberation fonts-dejavu'
...
check_apache_config() {
    ...
    if [ ! -f /etc/php/8.3/apache2/php.ini ]; then
        if [ -f /etc/php/8.3/cli/php.ini ]; then
            ...
            echo -e "${GREEN}✅ php.ini copied to /etc/php/8.3/apache2/${NC}"
        else
...
check_squid_traffic() {
    ...
    if [ ! -s /var/log/squid/access.log ]; then
        echo -e "${YELLOW}⚠️  /var/log/squid/access.log is empty${NC}"
        return 0
    fi
    ...
    if [ "$log_entries" -eq 0 ]; then
        echo -e "${YELLOW}⚠️  No valid Squid traffic found in access.log${NC}"
    else
        echo -e "${GREEN}✅ Found $log_entries valid traffic entries${NC}"
    fi
}
