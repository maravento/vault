...
if [ -n "$all_bans" ]; then
    ...
    iptables -I INPUT -i $lan -m set --match-set bandata src -p tcp -m multiport --dports 853,443 -j DROP
    iptables -I FORWARD -i $lan -m set --match-set bandata src -p tcp -m multiport --dports 853,443 -j DROP
