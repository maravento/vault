...
#
# If you accidentally close the RDP window and can't reconnect:
# Run: pkill -f xfreerdp3
...
install_winboat() {
    ...
    if ! command -v xfreerdp3 &> /dev/null; then
        apt-get update
        apt-get install -y freerdp3-x11
    else
        echo "FreeRDP3 is already installed. Skipping..."
    fi
    printf "\n"
