...
if [ -n "$all_bans" ]; then
    ...
    # Redirect HTTP (80/TCP) HTTP (Squid 3129 intercept) for bandata
    iptables -t nat -A PREROUTING -i $lan -m set --match-set bandata src -p tcp -m multiport --dports 80,3129 -j DNAT --to-destination $serverip:18880
    # Soft Block: Proxy (Squid 3128), DoT (TCP/853), HTTPs (TCP/443)
    iptables -I INPUT -i $lan -m set --match-set bandata src -p tcp -m multiport --dports 3128,853,443 -j DROP
    iptables -I FORWARD -i $lan -m set --match-set bandata src -p tcp -m multiport --dports 3128,853,443 -j DROP
