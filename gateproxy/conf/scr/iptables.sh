...
#sysctl -w net.ipv4.conf.default.accept_source_aclroute=1
...
# sysctl -w net.ipv4.conf.all.rp_filter=0 >/dev/null 2>&1 # opcional
...
# IPv6 DROP to lan
$ip6tables -A FORWARD -i "$lan" -j DROP
...
# SYADMIN
# Audit Ports: webmin (10000), sarg audit (18801)
aports="10000,18801"
...
#    $iptables -A INPUT -i $lan -p tcp --dport 8080 -m mac --mac-source $mac -j ACCEPT
#    $iptables -A FORWARD -i $lan -p tcp -m multiport --dports 443,8080 -m mac --mac-source $mac -j ACCEPT
#done

# MACLIMITED (Port 18800 to 3128 - Opcion 252 DHCP)
