...
#iptables -A INPUT -i $wan -s 192.168.0.0/16 -j DROP # WARNING
...
if [ $? -ne 0 ]; then
    ipset create macipset hash:ip,mac -exist
else
...
if [ $? -ne 0 ]; then
    ipset create dnsservers hash:ip -exist
else
...
if [ $? -ne 0 ]; then
    ipset create macdns hash:mac -exist
else
    ipset flush macdns
fi
...
if [ $? -ne 0 ]; then
    ipset create macdiscovery hash:mac -exist
else
